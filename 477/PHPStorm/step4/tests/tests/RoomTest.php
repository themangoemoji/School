...
class RoomTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function test_content() {
        ...
        $this->assertTrue($room->isEmpty(), "The room is empty - 49");
//        $this->assertFalse($room->contains(1), "50");

        $room->addContent(2);
//        $this->assertFalse($room->contains(1), "53");
//        $this->assertTrue($room->contains(2), "54");

        // Test one room away
        $room0 = new Wumpus\Room(1, 7);
        $room->addNeighbor($room0);
        $room0->addContent(3);
//        $this->assertFalse($room->contains(3), "60");
        $this->assertTrue($room->contains(3, 1),"61");
        ...
        $room->addNeighbor($room2);
        $room2->addContent(5);
        $this->assertFalse($room->contains(5),"68");
        $this->assertTrue($room->contains(5, 1),"69");

        // Test two rooms away
        $room1 = new Wumpus\Room(2, 8);
        $room0->addNeighbor($room1);
        $room1->addContent(4);
        $this->assertFalse($room->contains(4),"75");
        $this->assertFalse($room->contains(4, 1),"76");
        $this->assertTrue($room->contains(4, 2),"77");
    }
