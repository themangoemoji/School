...
class WumpusTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function test_move() {
        ...
        // Move to Wumpus
        $this->assertEquals(Wumpus\Wumpus::HAPPY, $wumpus->move(9), "h1");
        $this->assertEquals(Wumpus\Wumpus::HAPPY, $wumpus->move(8), "h2");
        $this->assertEquals(Wumpus\Wumpus::HAPPY, $wumpus->move(2), "h3");
        $this->assertEquals(Wumpus\Wumpus::EATEN, $wumpus->move(3), "w1");
        $this->assertFalse($wumpus->wasCarried());

        // Move to birds, will be picked up and moved to a pit in room ndx 15
        $wumpus = new Wumpus\Wumpus(self::SEED);
        $this->assertEquals(Wumpus\Wumpus::HAPPY, $wumpus->move(11), "h4");
        $this->assertEquals(Wumpus\Wumpus::HAPPY, $wumpus->move(18), "h5");
        $this->assertEquals(Wumpus\Wumpus::FELL, $wumpus->move(19), "f1");
        $this->assertTrue($wumpus->wasCarried(), "c2");

        // Move to pit
        $wumpus = new Wumpus\Wumpus(self::SEED);
        $this->assertEquals(Wumpus\Wumpus::HAPPY, $wumpus->move(11), "h6");
        $this->assertEquals(Wumpus\Wumpus::HAPPY, $wumpus->move(12), "h7");
        $this->assertEquals(Wumpus\Wumpus::HAPPY, $wumpus->move(4), "h8");
        $this->assertEquals(Wumpus\Wumpus::FELL, $wumpus->move(5), "f2");
        $this->assertFalse($wumpus->wasCarried());
    }
    ...
    public function test_shoot() {
        ...
        $this->assertEquals(3, $wumpus->numArrows(), "83");
        $this->assertTrue($wumpus->shoot(3), "84");

        $wumpus = new Wumpus\Wumpus(self::SEED);
        $wumpus->move(8);	// Two rooms away
        $this->assertFalse($wumpus->shoot(2), "88");
        $this->assertEquals(2, $wumpus->numArrows(), "89");
        $this->assertTrue($wumpus->shoot(2), "90");
        $this->assertEquals(1, $wumpus->numArrows(), "91");
    }
